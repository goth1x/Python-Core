"""
Для того чтобы выиграть главный приз лотереи, необходимо 
совпадение нескольких номеров на лотерейном билете с числами,
выпавшими случайным образом и в неком диапазоне во время 
очередного тиража. Например, необходимо угадать шесть чисел 
от 1 до 49 или пять чисел от 1 до 36 и т.д.

Напишите функцию, которая будет случайным образом подбирать 
набор чисел для лотерейного билета. Среди этих чисел не должно быть дубликатов.
...
def get_numbers_ticket(min, max, quantity):
    ...
    if min < quantity < max and min > 0 and max <= 1000:
        while len(ticket) < quantity:
            next_digit = randrange(min, max)
            ticket.append(next_digit) if next_digit not in ticket else None
    return sorted(ticket)
        
print(get_numbers_ticket(5, 20, 6))
